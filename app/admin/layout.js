    ...
    return (
        <div className="flex items-center gap-3 bg-[#121212] border border-white/10 rounded-xl px-3 py-2.5 shadow-sm mb-2">
            ...
            <div className="min-w-0">
                ...
                <div className="text-slate-300 text-xs font-medium truncate capitalize">Administrator</div>
            </div>
        </div>
    );
}
...
export default function AdminLayout({ children }) {
    ...
    return (
        <div className="min-h-screen bg-[#121212]/5 text-white flex flex-col md:flex-row font-sans">
            ...
            <div className="w-full md:w-64 border-b md:border-b-0 md:border-r border-white/10 bg-[#121212]/5 p-6 flex-col justify-between hidden md:flex h-screen sticky top-0">
                <div>
                    <div className="text-xl font-black text-white tracking-tight mb-5">HashPrime Admin</div>
                    <AdminAvatarBlock />
                        ...
                        {NAV_ITEMS.map((item) => {
                            ...
                            return (
                                <Link
                                    key={item.href}
                                    href={item.href}
                                    className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all relative z-10 ${isActive ? 'bg-[#d4af35]/10 text-white font-bold' : 'text-slate-500 font-medium hover:text-white hover:bg-slate-200/50'}`}
                                >
                                    ...
                                    {isActive && (
                                        <motion.div
                                            ...
                                            className="absolute left-0 w-1 h-8 bg-[#d4af35] rounded-r-full"
                                        />
                                    )}
                                </Link>
                            );
            ...
            {/* ── Main content ─────────────────────────────────────────────── */}
            <div className="flex-1 overflow-y-auto w-full relative min-h-screen bg-[#121212]/5">
                        ...
                        <div className="fixed top-6 left-4 right-4 z-50">
                            <header className="w-full bg-[#121212]/60 backdrop-blur-xl backdrop-saturate-150 border border-white/80 shadow-[0_8px_32px_0_rgba(31,38,135,0.08)] rounded-full px-5 flex items-center justify-between h-14">
                                ...
                                <Link href="/admin" className="flex items-center gap-2 shrink-0">
                                    <div className="w-6 h-6 rounded-md bg-[#d4af35] flex items-center justify-center">
                                        <span className="text-neon font-black text-xs">A</span>
                                    </div>
                                    <span className="font-black text-white text-base tracking-tight">Admin</span>
                                </Link>

                                {/* Current page label */}
                                <span className="text-xs font-bold text-slate-300 uppercase tracking-widest hidden sm:block">
                                    {NAV_ITEMS.find(i => i.href === pathname)?.name || 'Admin Panel'}
                                </span>
            ...
            >
                ...
                <div className="flex justify-between items-center px-6 py-5 border-b border-white/5">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="font-black text-white text-lg tracking-tight">Admin Console</span>
                    </div>
                    <button
                        ...
                        return (
                            <Link
                                ...
                                className={`offcanvas-link flex items-center gap-4 text-2xl font-black border-b pb-5 pt-3 transition-colors duration-200 ${isActive
                                    ? 'text-white border-[#d4af35]/20'
                                    : 'text-slate-300 border-white/5 hover:text-white'
                                    }`}
                            >
                                <Icon className={`w-6 h-6 shrink-0 ${isActive ? 'text-white' : 'text-slate-300'}`} />
                                {name}
                                {isActive && (
                                    <span className="ml-auto text-xs font-black bg-[#d4af35]/10 text-white px-2 py-0.5 rounded-full uppercase tracking-wider">Active</span>
                                )}
                            </Link>
                        );
                    })}
