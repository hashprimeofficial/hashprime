...
export default function DepositPage() {
    ...
    return (
        <div className="max-w-4xl space-y-8">
            <div>
                <h1 className="text-3xl font-black text-white mb-2 tracking-tight">Deposit Funds</h1>
            ...
            {!isKycApproved ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {/* Deposit Form */}
                    <div className="bg-[#121212] rounded-2xl p-6 border border-white/10 shadow-sm relative overflow-hidden">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5 relative z-10">
                            <div>
                                ...
                                <div className="relative">
                                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-300 font-bold text-base">
                                        {paymentMethod === 'usdt' ? '$' : '₹'}
                                    </div>
                                    <input
                                        type="number"
                                        ...
                                        required
                                        className="w-full bg-[#121212]/5 border border-white/10 rounded-lg pl-9 pr-4 py-3 text-white font-bold text-lg focus:outline-none focus:ring-2 focus:ring-neon focus:border-neon transition-all"
                            ...
                            <div>
                                <label className="block text-sm font-bold text-white mb-1.5">Payment Method</label>
                                <select
                                    value={paymentMethod}
                                    onChange={(e) => setPaymentMethod(e.target.value)}
                                    className="w-full bg-[#121212]/5 border border-white/10 rounded-lg px-4 py-3 text-white font-medium focus:outline-none focus:ring-2 focus:ring-neon focus:border-neon transition-all"
                                >
                            ...
                            <div className="my-4">
                                {paymentMethod === 'usdt' && (
                                    <>
                                        ...
                                        {qrExpanded && (
                                            <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#121212]/10/80 backdrop-blur-md"
                                                ...
                                                <div className="bg-[#121212] p-5 rounded-3xl shadow-2xl" onClick={e => e.stopPropagation()}>
                                                    ...
                                                    <p className="text-center text-xs font-bold text-slate-300 mt-3">USDC BEP20 Wallet QR</p>
                                        ...
                                        <div className="bg-[#d4af35] border border-[#d4af35]/20 rounded-2xl overflow-hidden"
                                            style={{ boxShadow: '0 0 40px rgba(57,255,20,0.04)' }}>
                                            {/* Header */}
                                            <div className="px-6 py-4 border-b border-white/[0.06] flex items-center justify-between">
                                                <div>
                                                    <p className="text-xs font-black text-[#d4af35] uppercase tracking-widest">USDC · BEP20</p>
                                                    <p className="text-slate-500 text-xs font-medium mt-0.5">Binance Smart Chain network</p>
                                                </div>
                                                <span className="text-xs font-black px-2.5 py-1 rounded-full"
                                                    style={{ background: 'rgba(57,255,20,0.1)', color: '#d4af35', border: '1px solid rgba(57,255,20,0.2)' }}>BSC</span>
                                            </div>

                                            {/* QR Code — centered, large */}
                                            <div className="flex flex-col items-center py-7 gap-2">
                                                <div className="relative group cursor-pointer" onClick={() => setQrMenu(v => !v)}>
                                                    <div className="bg-[#121212] p-3 rounded-2xl" style={{ boxShadow: '0 0 30px rgba(57,255,20,0.12)' }}>
                                                        <Image src="/qr.jpeg" alt="USDC BEP20 QR Code" width={200} height={200} className="rounded-xl" />
                                                    </div>
                                                    {/* Hover hint */}
                                                    <div className="absolute inset-0 rounded-2xl bg-[#121212]/10/0 group-hover:bg-[#121212]/10/25 transition-all flex items-center justify-center">
                                                        <span className="opacity-0 group-hover:opacity-100 transition-all text-white text-xs font-black bg-[#121212]/10/60 px-3 py-1.5 rounded-full">Tap for options</span>
                                                    </div>
                                                    {/* Context menu */}
                                                    {qrMenu && (
                                                        <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 z-20 bg-[#0F1929] border border-white/10 rounded-xl shadow-2xl overflow-hidden min-w-[160px]"
                                                            ...
                                                            <button onClick={() => { setQrExpanded(true); setQrMenu(false); }}
                                                                className="w-full flex items-center gap-2.5 px-4 py-3 text-xs font-bold text-slate-300 hover:bg-[#121212]/10 hover:text-white transition-all text-left border-b border-white/[0.06]">
                                                            ...
                                                            <a href="/qr.jpeg" download="HashPrime-USDC-BEP20-QR.jpeg"
                                                                ...
                                                                className="w-full flex items-center gap-2.5 px-4 py-3 text-xs font-bold text-slate-300 hover:bg-[#121212]/10 hover:text-white transition-all">
                                            ...
                                            <div className="px-6 pb-6 space-y-3">
                                                ...
                                                <div className="bg-[#121212]/[0.04] border border-white/[0.08] rounded-xl px-4 py-3">
                                                    <code className="text-[#d4af35] font-mono text-sm font-bold break-all leading-relaxed">
                                                ...
                                                <button type="button" onClick={copyAddress}
                                                    ...
                                                    style={{
                                                        background: copied ? '#d4af35' : 'rgba(57,255,20,0.08)',
                                                        color: copied ? '#0B1120' : '#d4af35',
                                ...
                                {paymentMethod === 'bank' && (
                                    <div className="bg-[#d4af35] border border-[#d4af35]/20 rounded-2xl p-5">
                                        <p className="text-xs font-black text-[#d4af35] uppercase tracking-widest mb-4">Bank Transfer Details</p>
                                ...
                                {paymentMethod === 'cash' && (
                                    <div className="bg-[#d4af35] border border-[#d4af35]/20 rounded-2xl p-5">
                                        <p className="text-xs font-black text-[#d4af35] uppercase tracking-widest mb-2">Cash Drop-off</p>
                                        <p className="text-slate-300 text-sm">Visit our office to submit cash. Upload the receipt photo below after payment.</p>
                                    </div>
                                )}
                            </div>

                            <div>
                                <label className="block text-sm font-bold text-white mb-1.5">Payment Screenshot / Receipt</label>

                                {previewUrl ? (
                                    <div className="relative rounded-xl overflow-hidden border border-white/10 bg-[#121212]/5 mb-3 group h-40">
                                        <img src={previewUrl} alt="Preview" className="w-full h-full object-contain" />
                                        <button type="button" onClick={() => { setPreviewUrl(''); setScreenshotBase64(''); setScreenshotFile(null); }} className="absolute inset-0 bg-[#121212]/10/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-white font-bold text-sm">
                                            Change Image
                                        </button>
                                    </div>
                                ) : (
                                    <label className="border-2 border-dashed border-white/10 rounded-xl p-8 flex flex-col items-center justify-center bg-[#121212]/5 hover:bg-[#d4af35]/5 hover:border-[#d4af35]/40 transition-all cursor-pointer">
                                        <ImageIcon className="w-8 h-8 text-slate-300 mb-3" />
                                        <span className="text-sm font-bold text-slate-200">Click to upload screenshot</span>
                                        <span className="text-xs text-slate-300 mt-1">Image must be &lt; 5MB</span>
                            ...
                            <motion.button
                                ...
                                className="w-full bg-neon text-white font-black py-4 rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-neon/20 hover:shadow-neon/40 disabled:opacity-50"
                            >
                    ...
                    <div>
                        ...
                        {!depositsData ? (
                            <div className="animate-pulse space-y-4">
                                {[1, 2, 3].map(i => <div key={i} className="h-20 bg-slate-100 rounded-xl border border-white/10" />)}
                            </div>
                        ) : depositsData.deposits?.length === 0 ? (
                            <div className="text-center py-10 bg-[#121212]/5 rounded-2xl border border-white/10 text-slate-500 font-medium">
                                No deposits found.
                            </div>
                        ) : (
                            <div className="space-y-3">
                                {depositsData.deposits.map(dep => (
                                    <div key={dep._id} className="bg-[#121212] rounded-xl p-4 border border-white/10 shadow-sm flex items-center justify-between group hover:border-neon/50 transition-all">
                                        <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 rounded-full bg-[#121212]/5 flex items-center justify-center border border-white/5 shadow-inner">
                                        ...
                                        {dep.screenshotUrl && (
                                            <a href={dep.screenshotUrl} target="_blank" rel="noopener noreferrer" className="ml-4 shrink-0">
                                                <div className="w-16 h-16 rounded-lg overflow-hidden border border-white/10 bg-[#121212]/5 grayscale hover:grayscale-0 transition-all opacity-80 hover:opacity-100">
