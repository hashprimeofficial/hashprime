...
export default function LoginPage() {
    ...
    const inputClass = "w-full bg-[#121212] border border-white/10 rounded-xl px-4 py-3.5 text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:border-transparent transition-all font-medium shadow-sm text-sm";
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="space-y-1.5">
                    <div className="flex justify-between items-center">
                        ...
                        <Link href="/forgot-password" className="text-xs font-bold text-slate-300 hover:text-slate-700 transition-colors">Forgot password?</Link>
                    </div>
                ...
                <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} type="submit" disabled={loading}
                    ...
                    style={{ background: 'linear-gradient(135deg, #d4af35, #22c55e)', color: '#0b1120', boxShadow: '0 8px 24px rgba(57,255,20,0.25)' }}>
                    {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <>Sign in <ArrowRight className="w-4 h-4" /></>}
                </motion.button>
            </form>

            <div className="mt-8 pt-6 border-t border-white/5 text-center">
                <p className="text-xs text-slate-300 font-medium">Secured by end-to-end encryption</p>
            </div>
        </motion.div>
    );
}
