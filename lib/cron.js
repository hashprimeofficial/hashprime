        ...
        for (const inv of activeInvestments) {
            ...
            for (let m = 1; m <= 6; m++) {
                ...
                if (today >= payoutDate) {
                    ...
                    // 1. Increment User's INR Wallet atomically
                    await User.findByIdAndUpdate(user._id, {
                        $inc: { inrWallet: monthlyYield }
                    });

                    // 2. Create transaction record for investor
                    ...
                    // 4. L2 Referral Commission: 5% of this month's payout to the referrer
                    if (user.referredBy) {
                        ...
                        if (referrer && referrer._id.toString() !== user._id.toString()) {
                            const l2Commission = Math.round(monthlyYield * 0.05);
                            if (l2Commission > 0) {
                                await User.findByIdAndUpdate(referrer._id, {
                                    $inc: { referralWalletInr: l2Commission }
                                });
                                await Transaction.create({
                                    userId: referrer._id,
                                    type: 'referral_bonus',
                                    amount: l2Commission,
                                    currency: 'INR',
                                    description: `Monthly Referral Commission — ${user.name} ${monthNames[m - 1]}`
                                });
                                console.log(`[Cron] L2 referral commission ₹${l2Commission} credited to ${referrer.email} for ${user.email}'s ${monthNames[m - 1]}`);
                            }
                        }
                    }
                }
    ...
    try {
        ...
        for (const investment of maturedInvestments) {
            ...
            if (investment.schemeType === 'limited_inr') {
                ...
                rewardToPay = 0; // monthly rewards already processed
                
                if (forcePayYieldTotal > 0) {
                    await User.findByIdAndUpdate(user._id, {
                        $inc: { inrWallet: forcePayYieldTotal }
                    });
                }
            } else {
            ...
            // 1. Credit the User's specific Wallet Balance atomically
            await User.findByIdAndUpdate(user._id, {
